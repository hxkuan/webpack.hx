(function($) {
	...
	var my_confirm_box, my_loading_box,my_toast_box;

	var my_toast_name="mytoast";
	...
	$[my_toast_name] = function(options) {
		if(my_toast_box == null) {
			my_toast_box = $("<div class='my-toast-box'><div class='my-loading-title' style='padding-top: 0'></div></div>");
			$("body").append(my_toast_box);
		}
		if(typeof(options) == "string" && options == "getDialog") {
			return my_toast_box;
		}
		if(typeof(options) == "string" && options == "hide") {
			my_toast_box.mydialog("hide");
			return;
		}
		if(typeof options === 'undefined') {
			options = {};
		}
		options.loading_box = my_toast_box;
		new MyJqMyLoad(options);
	};
}(jQuery));